...
if uploaded_file is not None:
    ...
    if ret:
        ...
        smoothed_intensity_filtered = smoothed_intensity[mask]
        ...
        fig.add_trace(go.Scatter(x=time_axis_filtered, y=smoothed_intensity_filtered, mode="lines", name="Smoothed signal", line=dict(color="blue")))
        if len(peaks) > 0:
            fig.add_trace(go.Scatter(x=time_axis_filtered[peaks], y=smoothed_intensity_filtered[peaks], mode="markers", name="Detected peaks", marker=dict(color="red", size=7, symbol="cross")))
