...
if uploaded_file is not None:
    ...
    if ret:
        ...
        st.image(frame_rgb, caption="Preview of the calculating zone (red zone)")
        ...
        col1.metric("Frequency (Hz)", f"{freq:.5f}")
        ...
        #st.pyplot(fig)
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=time_axis, y=intensity, mode="lines", name="Intensity", line=dict(color="blue")))
        if len(peaks) > 0:
